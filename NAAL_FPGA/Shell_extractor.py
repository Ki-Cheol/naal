...
class host_Init(object):
    ...
    def __init__(self, fpga_name, max_attempts=5, timeout=5):
        ...
        ##socket
        self.tcp_port =int(self.config['ssh_port'])
    
        #self.tcp_init = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        #print(str(config_FPGA.host_config['ip'])+" port : ",str(self.tcp_port)+"socket create  test = 8080")
        #self.tcp_init.bind((config_FPGA.host_config['ip'], 8080))
        #self.tcp_init.listen(1)  # Ready to accept a connection
        #self.tcp_recv = None  # Placeholder until socket is connected
       # self.udp_init =sockets.UDPReceiveSocket((config_FPGA.host_config['ip'],5001))
        self.tcp_init= socket.socket(socket.AF_INET, socket.SOCK_STREAM)
